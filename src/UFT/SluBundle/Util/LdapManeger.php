...
/**
 * Class LdapManeger
 * https://github.com/ibspoof/LDAP_ORM
 * @package UFT\SluBundle\Util
 */
...
class LdapManeger
{
    ...
    public function novoOld($host, $port = 389, $ldap_version = 3)
    {
    ...
    /**
     * @param string $host          server IP or domain
     * @param string $user          user domain
     * @param string $pass          pw domain
     * @param int $port             port   default: 389
     * @param int $ldap_version     ldap version to use     default: 3
     */

    public function __construct($host, $user, $pass, $baseDn = null, $port = 389, $ldap_version = 3)
    {
        ldap_set_option(NULL, LDAP_OPT_PROTOCOL_VERSION, $ldap_version);
        $this->server = $host;
        $this->port = $port;
    ...
    protected function _build_hasMany($result = array(), $hasMany = array())
    {
        ...
        foreach ($result as $k => $v) {
            ...
            foreach ($v as $base => $data) {
                ...
                if (isset($hasMany['fields']) &&  !empty($hasMany['fields'])  ) {
    ...
    public function save($dn = false, $data = array(), $strict = false)
    {
        ...
        if ($this->dnExists($dn)) {
            ...
            return $this->update($dn,$data,$strict);
        }
    ...
    public function update($dn = false, $data = array(), $strict = false)
    {
        ...
        if (isset($data['objectClass']) || isset($data['objectclass'])) {
...
//    public function addMail($uid, $mail){
//        $dn = $this->dnBuilder(array('uid' => $uid), 'ou=People,' . $this->baseDn);
//        $entity = $this->findByDn($dn);
//        $array_mail['mail'] = null;
//        if (count($entity['mail']) == 1){
//            $array_mail['mail'][0] = $entity['mail'];
//        }else{
//            $array_mail['mail'] = $entity['mail'];
//        }
//        $array_mail['mail'][count($array_mail['mail'])] = $mail;
//        ldap_modify($this->_connection, $dn, $array_mail);
//    }
    public function addMail($uid, $mail){
        $dn = $this->dnBuilder(array('uid' => $uid), 'ou=People,' . $this->baseDn);
        $array_mail['postalAddress'] = $mail;
        ldap_modify($this->_connection, $dn, $array_mail);
    }
    public function addMember($cn, $members){
        if( is_string($members) && strpos(strtolower($members), 'ou=desativados') !== false) {
            return false;
        }
        $dn = $this->dnBuilder(array('cn' => $cn), 'ou=Group,' . $this->baseDn);
        $array_member['member'] = $members;
        return @ldap_mod_add($this->_connection, $dn, $array_member);
    }

    public function verifyGroupMember($entity, $entityManager, $cn)
    {
        $auxiliar = $entityManager->getRepository(NewGrupoLdap::class)->findOneByCn($cn);
        if (!empty($auxiliar) && !$auxiliar->hasMember($entity->getDn()) && $entity->isRecadastrado()) {
            if (!$this->addMember($cn, $entity->getDn())) {
                $this->showError();
            }
        }
    }
    public function populeGroupMember($entity, $entityManager) {
        if ($entity->getAluno()) {
            $this->verifyGroupMember($entity, $entityManager, "Alunos");
        }
        if ($entity->getProfessor()) {
            $this->verifyGroupMember($entity, $entityManager, "Professores");
        }
        if ($entity->getFuncionario()) {
            $this->verifyGroupMember($entity, $entityManager, "Técnicos");
        }
        $this->verifyGroupMember($entity, $entityManager, $entity->getCampus());
    }
