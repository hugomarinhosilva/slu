 ...
 */
class NewGrupoLdap extends Group
{
...
//    private $id;
    ...
    public function hasMember($dn)
    {
        if(in_array(strtolower($dn),$this->member) || in_array($dn,$this->member)){
            return true;
        }
        return false;
    }
     ...
     *
     * @param PessoaLdap $member the mmeber to add
     */
    public function addMember($member)
    {
        if($member instanceof PessoaLdap){
            if(!$this->hasMember($member->getDefaultDn())){
                $this->member[] = $member->getDefaultDn();
            }
        }else{
     ...
     *
     * @param PessoaLdap $member the mmeber to remove
     */
