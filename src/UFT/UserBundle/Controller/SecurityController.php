...
use UFT\UserBundle\Exception\CaptchaException;
...
class SecurityController extends Controller
{
    public function loginoldAction(Request $request)
    {
        ...
        if ($request->isMethod('POST')) {
            $liberado = $request->getSession()->get($lastUsernameKey);
            // validate the user-entered Captcha code when the form is submitted
            $recaptcha = new ReCaptcha('6Lf5dhQUAAAAAKPlAtrhHfwmMkpRVYYaSJFaCzqs');
            $resp = $recaptcha->verify($request->request->get('g-recaptcha-response'), $request->getClientIp());
            if ($resp->isSuccess() || ($liberado == null)) {
                return $this->redirectToRoute('fos_user_security_check', [
                    'request' => $request], 307);
            } else {
                $invalidCaptchaEx = new CaptchaException();
